...
contract Nft is ERC721, ERC721Enumerable {
  ...
  function mint(string memory _fileName, string memory _fileHash, string memory _fileType, uint _fileSize) external {
    bytes32 fileType = keccak256(bytes(_fileType));
    ...
    require(
      fileType == keccak256(bytes("image/jpeg")) 
      || fileType == keccak256(bytes("image/png")) 
      || fileType == keccak256(bytes("image/tiff")) 
      || fileType == keccak256(bytes("image/gif"))
    );
    require(_fileSize > 0);
